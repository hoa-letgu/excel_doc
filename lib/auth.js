...
// No ON DELETE CASCADE in the schema (db.js) — clear dependent rows by hand.
const deleteUserStmt = db.prepare(`DELETE FROM users WHERE id = ?`);
const deleteUserCleanup = db.transaction((userId) => {
  db.prepare(`DELETE FROM sessions WHERE user_id = ?`).run(userId);
  db.prepare(`DELETE FROM workbook_permissions WHERE user_id = ?`).run(userId);
  db.prepare(`DELETE FROM sheet_permissions WHERE user_id = ?`).run(userId);
  deleteUserStmt.run(userId);
});
function deleteUser(userId) {
  deleteUserCleanup(userId);
}
