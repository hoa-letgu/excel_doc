  ...
  return db
    .prepare(
      `SELECT s.id, s.name, s.workbook_id AS workbookId, w.name AS workbookName,
         s.config, (SELECT MAX(col) + 1 FROM cells WHERE sheet_id = s.id) AS usedColumns
       FROM sheets s
       JOIN workbooks w ON w.id = s.workbook_id
       ORDER BY w.id, s.order_index`
    )
    .all()
    .map(({ config, usedColumns, ...sheet }) => {
      let columnCount = 26;
      try { columnCount = Number(JSON.parse(config || '{}').columnCount) || 26; } catch {}
      return { ...sheet, columnCount: Math.max(26, columnCount, usedColumns || 0) };
    });
}
...
function updateWorkbookLink(id, link) {
  return db.prepare(`UPDATE workbook_links SET
    left_workbook_id = ?, left_sheet_id = ?, left_key_col = ?, left_value_col = ?,
    right_workbook_id = ?, right_sheet_id = ?, right_key_col = ?, right_value_col = ?, bidirectional = ?
    WHERE id = ?`).run(link.leftWorkbookId, link.leftSheetId, link.leftKeyCol, link.leftValueCol,
      link.rightWorkbookId, link.rightSheetId, link.rightKeyCol, link.rightValueCol, link.bidirectional ? 1 : 0, id).changes > 0;
}
  ...
  return db
    .prepare(
      `SELECT
         ...
         bidirectional AS bidirectional
       FROM workbook_links
       WHERE left_sheet_id = ? OR right_sheet_id = ?
       ORDER BY id`
    )
    .all(sheetId, sheetId)
    .map((link) => ({ ...link, bidirectional: !!link.bidirectional }));
...
function copyLinkedValue(direction, sourceRow, visited = new Set(), onlyTargetRow) {
  ...
  for (const targetRow of targetRows) {
    if (onlyTargetRow !== undefined && targetRow !== onlyTargetRow) continue;
    const key = JSON.stringify([direction.targetSheetId, targetRow, direction.targetValueCol]);
    if (visited.has(key)) continue;
    visited.add(key);
    // Explicit nulls clear old values/formulas in the live cell as well as SQLite.
    const cellData = { v: null, f: null, p: null, si: null, ...(sourceCell?.value ? JSON.parse(sourceCell.value) : {}) };
    cellData.f = sourceCell?.formula ?? null;
    upsertCell(direction.targetSheetId, targetRow, direction.targetValueCol, JSON.stringify(cellData), cellData.f);
    updates.push({
...
function clearLinkedValuesForKey(direction, sourceKey, visited = new Set()) {
  if (!sourceKey) return [];
  // A duplicate source key still represents this record.
  if (findRowsByCellValue(direction.sourceSheetId, direction.sourceKeyCol, sourceKey).length) return [];
  const updates = [];
  for (const row of findRowsByCellValue(direction.targetSheetId, direction.targetKeyCol, sourceKey)) {
    const key = JSON.stringify([direction.targetSheetId, row, direction.targetValueCol]);
    if (visited.has(key)) continue;
    visited.add(key);
    const target = getCell(direction.targetSheetId, row, direction.targetValueCol);
    const cellData = { ...JSON.parse(target?.value || '{}'), v: null, f: null, p: null, si: null };
    upsertCell(direction.targetSheetId, row, direction.targetValueCol, JSON.stringify(cellData), null);
    updates.push({ workbookId: direction.targetWorkbookId, sheetId: direction.targetSheetId, row, col: direction.targetValueCol, cellData });
  }
  return updates;
}

function applyWorkbookLinksForEdit(sourceSheetId, changedCellValue, previousCells = []) {
  const updates = [];
  const queue = [];
  const visited = new Set();
  const links = listLinksForSheet(sourceSheetId);
  const previous = new Map(previousCells.map(cell => [`${cell.row}:${cell.col}`, cell]));
  const has = (cols, col) => Object.prototype.hasOwnProperty.call(cols, col);
  for (const [rowText, cols] of Object.entries(changedCellValue || {})) {
    if (!cols) continue;
    const row = Number(rowText);
    for (const link of links) {
      const isLeft = link.leftSheetId === sourceSheetId;
      const keyCol = isLeft ? link.leftKeyCol : link.rightKeyCol;
      const valueCol = isLeft ? link.leftValueCol : link.rightValueCol;
      const outgoing = directionForLink(link, sourceSheetId);
      const incoming = directionForLink(link, isLeft ? link.rightSheetId : link.leftSheetId);
      const keyChanged = has(cols, keyCol);
      const valueChanged = has(cols, valueCol);
      if (!keyChanged && !valueChanged) continue;
      const key = cellDisplayValue(getCell(sourceSheetId, row, keyCol));
      if (!key) {
        if (keyChanged && outgoing && !link.bidirectional) {
          const oldKey = cellDisplayValue(previous.get(`${row}:${keyCol}`));
          updates.push(...clearLinkedValuesForKey(outgoing, oldKey, visited));
        }
        continue;
      }
      const cell = getCell(sourceSheetId, row, valueCol);
      const hasValue = !!cell?.formula || cellDisplayValue(cell) !== '';
      // A newly entered key joins an existing record. Hydrate this row only;
      // never push its blank/stale cells back into the established record.
      if (keyChanged && incoming && (!outgoing || !valueChanged || !hasValue)) {
        const matches = findRowsByCellValue(incoming.sourceSheetId, incoming.sourceKeyCol, key);
        if (matches.length) {
          visited.add(JSON.stringify([incoming.sourceSheetId, matches[0], incoming.sourceValueCol]));
          updates.push(...copyLinkedValue(incoming, matches[0], visited, row));
          continue;
        }
      }
      if (outgoing && (valueChanged || keyChanged) && (!keyChanged || hasValue)) {
        visited.add(JSON.stringify([sourceSheetId, row, valueCol]));
        queue.push({ sheetId: sourceSheetId, cellValue: { [row]: { [valueCol]: cols[valueCol] ?? {} } } });
      }
    }
  }
  queue.push(...updates.map(update => ({ sheetId: update.sheetId, cellValue: { [update.row]: { [update.col]: update.cellData } } })));
  return [...updates, ...propagateWorkbookLinks(queue, visited)];
}

function propagateWorkbookLinks(queue, visited = new Set()) {
  const updates = [];
  for (const { sheetId, cellValue } of queue) {
    for (const [row, cols] of Object.entries(cellValue || {})) {
      for (const col of Object.keys(cols || {})) visited.add(JSON.stringify([sheetId, Number(row), Number(col)]));
    }
  }
  // Every cell is written at most once per edit, even in cyclic two-way links.
  for (let index = 0; index < queue.length; index++) {
    const { sheetId, cellValue } = queue[index];
    for (const link of listLinksForSheet(sheetId)) {
      const direction = directionForLink(link, sheetId);
      if (!direction) continue;
      for (const [row, cols] of Object.entries(cellValue || {})) {
        if (!cols || !Object.prototype.hasOwnProperty.call(cols, direction.sourceValueCol)) continue;
        for (const update of copyLinkedValue(direction, Number(row), visited)) {
          updates.push(update);
          queue.push({ sheetId: update.sheetId, cellValue: { [update.row]: { [update.col]: update.cellData } } });
        }
      }
    }
  }
...
function applyWorkbookLinksForResync(sheetId, previousCells) {
  const currentCells = getCells(sheetId);
  const previous = new Map(previousCells.map(cell => [`${cell.row}:${cell.col}`, cell]));
  const changes = {};
  const cleared = [];
  const visited = new Set();
  for (const link of listLinksForSheet(sheetId)) {
    const left = link.leftSheetId === sheetId;
    const keyCol = left ? link.leftKeyCol : link.rightKeyCol;
    const valueCol = left ? link.leftValueCol : link.rightValueCol;
    const oldRows = new Map();
    for (const cell of previousCells.filter(cell => cell.col === keyCol && cellDisplayValue(cell) !== '')) {
      const key = cellDisplayValue(cell);
      if (!oldRows.has(key)) oldRows.set(key, []);
      oldRows.get(key).push(cell.row);
    }
    if (left && !link.bidirectional) {
      const direction = directionForLink(link, sheetId);
      for (const key of oldRows.keys()) cleared.push(...clearLinkedValuesForKey(direction, key, visited));
    }
    for (const cell of currentCells.filter(cell => cell.col === keyCol)) {
      const key = cellDisplayValue(cell);
      if (!key) continue;
      const current = getCell(sheetId, cell.row, valueCol);
      const candidates = oldRows.get(key) || [];
      const matchIndex = candidates.findIndex(row => {
        const candidate = previous.get(`${row}:${valueCol}`);
        return cellDisplayValue(candidate) === cellDisplayValue(current) && (candidate?.formula ?? null) === (current?.formula ?? null);
      });
      const oldRow = candidates.splice(matchIndex < 0 ? 0 : matchIndex, 1)[0];
      const old = oldRow === undefined ? undefined : previous.get(`${oldRow}:${valueCol}`);
      // Compare by key, not row number: inserting/deleting/sorting rows must
      // not treat shifted records as edits or delete their remote partners.
      if (oldRow === undefined) (changes[cell.row] ??= {})[keyCol] = JSON.parse(cell.value || '{}');
      if (cellDisplayValue(current) !== cellDisplayValue(old) || (current?.formula ?? null) !== (old?.formula ?? null)) {
        (changes[cell.row] ??= {})[valueCol] = JSON.parse(current?.value || '{}');
      }
    }
  }
  const updates = applyWorkbookLinksForEdit(sheetId, changes);
  const queue = cleared.map(update => ({ sheetId: update.sheetId, cellValue: { [update.row]: { [update.col]: update.cellData } } }));
  return [...cleared, ...updates, ...propagateWorkbookLinks(queue, visited)];
}
...
function syncWorkbookLink(id) {
  ...
  for (const row of sourceRows) {
    if (link.bidirectional) {
      const cell = getCell(direction.sourceSheetId, row, direction.sourceValueCol);
      if (!cell?.formula && cellDisplayValue(cell) === '') {
        const key = cellDisplayValue(getCell(direction.sourceSheetId, row, direction.sourceKeyCol));
        const populatedRow = findRowsByCellValue(direction.targetSheetId, direction.targetKeyCol, key)
          .find(targetRow => {
            const target = getCell(direction.targetSheetId, targetRow, direction.targetValueCol);
            return target?.formula || cellDisplayValue(target) !== '';
          });
        if (populatedRow !== undefined) {
          updates.push(...copyLinkedValue(directionForLink(link, link.rightSheetId), populatedRow, new Set(), row));
        }
      }
    }
    updates.push(...copyLinkedValue(direction, row));
  }
  return [...updates, ...propagateWorkbookLinks(updates.map((update) => ({
    sheetId: update.sheetId, cellValue: { [update.row]: { [update.col]: update.cellData } },
  })))];
}
